...
@Service
@Properties({
		@Property(name = Constants.SERVICE_DESCRIPTION, value = "PaneraImageRenditions"),
		@Property(name = Constants.SERVICE_VENDOR, value = "Adobe"),
		@Property(name = "process.label", value = "Panera Image Renditions") })
public class PaneraImageRenditions extends AbstractAssetWorkflowProcess
		...
		try {
			...
			for (final String singlePercentage : percentages) {
				...
				if ("desktop".equals(originalSizes[0].toString())) {
					...
					if (payloadPath
							.startsWith((String) HdsCorpGlobalConfiguration
									.getPropertyValue(HdsCorpGlobalConfiguration.ARTICLE_DETAILS_IMAGES_DAM_PATH))
							&& "articledetail".equals(originalSizes[0].toString())) {

						long articlewidth = Long.parseLong(originalSizes[1].toString());
						float factor  = (float) width / articlewidth;
						long articleHeight = (long) (length/ factor);
						rendition = articlewidth + ":" + articleHeight;
						rendtionList.add(rendition);
						renditionNames.put("cq5dam.web."+articlewidth+"."+articleHeight, originalSizes[0].toString());
	...
	private String[] buildArguments(final MetaDataMap metaData) {
		final String processArgs = (String) metaData.get(
				PaneraImageRenditions.Arguments.PROCESS_ARGS.name(), String.class);
		if (StringUtils.isNotBlank(processArgs)) {
			return processArgs.split(ARGUMENT_SEPARATOR);
		}

		final String[] configs = (String[]) metaData.get(PaneraImageRenditions.Arguments.CONFIGS.name(),
				String[].class);
		if (configs != null) {
			return configs;
		}
